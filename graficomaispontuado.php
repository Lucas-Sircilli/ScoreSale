<?php
	...
	try {
		...
		$grafico = (array)  getGraficomaisPontuado($receive,$conn);
		//echo $grafico[0]['SUM(pontuacoes)'];
		//return;
		
		if($grafico != null){
			$resp->resultadoOp = "Operação Realizada com sucesso!";
			$resp->pontuacoes = $grafico[0]['pontuacoes'];
			$resp->id_parceiros = $grafico[0]['id_parceiros'];
			//$resp->SUM(pontuacoes) = $grafico[0]['SUM(pontuacoes)'];
			//$resp->id_parcerios = $grafico[1]['id_parcerios'];
